...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "ALS|Character States")
	void SetStoredGait(EALSGait NewGait) { StoredGait = NewGait; }

	UFUNCTION(BlueprintGetter, Category = "ALS|Character States")
	EALSGait GetStoredGait() const { return StoredGait; }
	...
	UFUNCTION(BlueprintCallable, Category = "ALS|Character States")
	void SetAttackState(EALSAttackState NewState);

	/*UFUNCTION(BlueprintCallable, Server, Reliable, Category = "ALS|Character States")
	void Server_SetAttackState(EALSAttackState NewState);*/

	UFUNCTION(BlueprintGetter, Category = "ALS|Character States")
	EALSAttackState GetAttackState() const { return AttackState; }
	...
	/** Implement on BP to get required attack animation according to character's state */
	UFUNCTION(BlueprintCallable, BlueprintImplementableEvent, Category = "ALS|Input")
	...
	// Attacking
	/** Yeurgh. Must be a better way to block attack spamming input.
	 * Called in AnimBP via AnimEvent.
	 */
	UFUNCTION(BlueprintCallable, Category = "ALS|Attacking Timers")
	void SetAttackingTimer(float Time);
...
protected:
	...
	virtual void OnAttackStateChanged(EALSAttackState PreviousState);
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "ALS|State Values") //ReplicatedUsing = OnRep_OverlayState
	EALSAttackState AttackState = EALSAttackState::Resting;
...
private:
	...
	// Setting SetOverlayState(EALSOverlayState::MeleeUnarmed) and SetRotationMode(EALSRotationMode::Aiming) in AimPressedAction
	...
	FTimerDelegate AttackStateTimerDelegate;

	// Debugs
public:

	UFUNCTION(BlueprintNativeEvent, Category = "ALS|Animation")
	void CallBlueprintDebugPrint(FVector Vector);
};
